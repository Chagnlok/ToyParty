Shader "Custom/ProteanClouds" 
{
	...
	SubShader 
    {
        ...
        Pass
        {
            ...
            #define linstep( mn,  mx, x)   (clamp((x - mn)/(mx - mn), 0., 1.))
            float prm1 = 0.;
            

            vec2 disp(float t){ return vec2(sin(t*0.22)*1., cos(t*0.175)*1.)*2.; }

            vec2 bsMo = vec2(0.0, 0.0);
...
vec3 iLerp(in vec3 a, in vec3 b, in float x)
{
    ...
    return clamp(ic,0.,1.);
            ...
            fixed4 frag(v2f IN) : COLOR
            {
                ...
                vec2 fragCoord = IN.fragcoord;
                vec2 p  = (fragCoord.xy - halfResolution)/iResolution.y;
